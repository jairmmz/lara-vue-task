<template>
    <div class="block completed-task rounded-lg bg-slate-50 p-6 text-surface shadow-secondary-1 dark:bg-surface-dark">
        ...
        <div v-for="task in projectData.data?.tasks" :key="task.id" 
            ...
            @drag="emit('fromCompletedToPending', task.id, projectData.data?.project.id)"
            draggable="true" :class="`task-card completed-task-${task.id}`">
...
const emit = defineEmits<{
    (event: 'fromCompletedToPending', taskId: string, projectId: string): Promise<void>;
}>();
</script>
